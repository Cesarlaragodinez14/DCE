...
<head>
    <meta charset="UTF-8">
    <title>Validación de Archivo de Auditoría</title>
...
<body>
    <div class="container">
        ...
        <h1>Archivo Verificado</h1>
        <!-- Información de Validación -->
        <div class="info">
            <p><strong>Hash:</strong><span>{{ $hash }}</span></p>
            <p><strong>Correo del Generador:</strong><span>{{ $userEmail }}</span></p>
            <p><strong>Dirección IP del usuario al momento de generación:</strong><span>{{ $ipAddress }}</span></p>
            <p><strong>Fecha y Hora de Generación:</strong><span>{{ \Carbon\Carbon::parse($generatedAt)->format('d/m/Y H:i:s') }}</span></p>
            <p><strong>Clave de Acción:</strong><span>{{ $auditoria->clave_de_accion }}</span></p>
        </div>
        <!-- Botón de Descarga -->
        <div class="download-button">
            <a href="{{ route('validador.download', ['hash' => $hash]) }}">Descargar PDF</a>
        </div>
    </div>
</body>
</html>
